...
class ValidEventShapeFilterTest {
    ...
    @ParameterizedTest
    @NullAndEmptySource
    void shouldContinueChain_WhenEventIsFinalAndDoesNotDeclareNextExpectedEvent(String nextExpectedEvent) {
    ...
    @ParameterizedTest
    @NullAndEmptySource
    void shouldContinueChain_WhenEventDeclaresNeitherTtlNorNextExpectedEvent(String nextExpectedEvent) {
        var chain = mock(EventIngestionFilterChain.class);
    ...
    private EventIngestionContext buildContext(boolean finalEvent, String nextExpectedEvent, Integer nextEventTtlSeconds) {
        var event = new Event();
